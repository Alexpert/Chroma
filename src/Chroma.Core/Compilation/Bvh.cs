...
/// <summary>
/// A bounding volume hierarchy over a list of boxes, flattened for a shader that has no stack.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <b>It is built over boxes and knows nothing else about them.</b> That was true of it while it
/// served one caller and is worth saying now that it serves two: iteration 24's <c>mesh</c> builds
/// a second tree with the same call, over <i>triangles</i> rather than placements, and the only
/// difference is what a leaf's index means to whoever reads it back. The two trees are also
/// written to the GPU in the same two texels, so <see cref="GpuLayout.NodeStride"/> serves both.
/// </para>
/// <para>
...
/// </para>
/// <para>
/// <b>One item per leaf.</b> A leaf holding four would need a count and a loop; a leaf holding
/// one needs neither, and the extra levels of tree cost two texels each. The simpler shader is
/// worth more than the shallower tree, and that holds for a bunny's triangles as squarely as for
/// a chess set's pieces.
/// </para>
/// </remarks>
public static class Bvh
{
    ...
    /// <param name="Item">
    /// What this leaf holds, as a position in <see cref="Result.Order"/>, or -1 for an interior
    /// node. An instance to the scene's tree and a triangle to a mesh's.
    /// </param>
    public readonly record struct Node(Aabb Bounds, int Escape, int Item);

    /// <param name="Order">
    /// Which item each slot holds, as an index into what <see cref="Build"/> was given. The build
    /// permutes them so that a leaf's item sits where the tree wants it, and the packer writes
    /// its table in this order.
    /// </param>
    ...
    private sealed class Builder(IReadOnlyList<Aabb> bounds, Vector3[] centroids, int[] order)
    {
        ...
        private int Split(int lo, int hi, Aabb box)
        {
            ...
            if (bestAxis < 0)
            {
                ...
                // range still has to be cut -- a leaf holds one item -- so sort along the
